...
def _create_content_loss(p, f):
    ...
    Inputs:
        p, f: np.ndarrays are just P, F in the paper are np.ndarrays
    ...
    """
    return 1. / (4 * p.size) * tf.reduce_sum((p - f) ** 2, name='content-loss')
...
def _gram_matrix(F, N, M):
    ...
    """
    f = tf.reshape(F, shape=[N, M])
    return tf.matmul(f, f, transpose_a=False, transpose_b=True, name='gram-matrix')


def _single_style_loss(a, g):
    """ Calculate the style loss at a certain layer
    Inputs:
        a: np.ndarray: is the feature representation of the real image
        g: np.ndarrays: is the feature representation of the generated image
    Output:
    ...
    gram_b = _gram_matrix(g, N, M)
    return tf.reduce_sum((gram_a - gram_b) ** 2 / (2 * N * M) ** 2, name='single-syle-loss')
...
def _create_style_loss(A, model):
    ...
    return tf.add_n([E[i] * W[i] for i in range(n_layers)], name='style-loss')
    ...
    with tf.Session() as sess:
        ...
        for index in range(initial_step, ITERS):
            ...
            if (index + 1) % skip_step == 0:
                ...
                filename = 'outputs/' + CONTENT + '-' + STYLE + '-' + '%d.png' % (index)
